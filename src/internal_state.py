...
from src.pipeline_explorer.explorer import PipelineDependencyVisualizer, PipelineUnverseMap

_PAT = os.environ.get("PAT")
_ORGANIZATION_NAME = os.environ.get("ORGANIZATION_NAME")
_PROJECT = os.environ.get("PROJECT")

_CONSTRUCTED_PIPELINE_DEPENDENCY_VISUALIZER: Union[PipelineDependencyVisualizer, None] = None
...
def get_state(force = False) -> Dict[str, PipelineDependencyVisualizer]:
    ...
    if _CONSTRUCTED_PIPELINE_DEPENDENCY_VISUALIZER is None or force:
        ...
        _CONSTRUCTED_PIPELINE_DEPENDENCY_VISUALIZER = PipelineDependencyVisualizer(pipeline_universe_map)
        _CONSTRUCTED_PIPELINE_DEPENDENCY_VISUALIZER.build_graph()

    state["pipeline_dependency_visualizer"] = _CONSTRUCTED_PIPELINE_DEPENDENCY_VISUALIZER
    return state
